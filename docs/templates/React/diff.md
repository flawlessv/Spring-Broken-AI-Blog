...
在深入讲解 Diff 算法之前，我们需要先理解两个重要概念：**虚拟 DOM** 和 **Fiber 架构**。它们是 Diff 算法发挥作用的基础。
...
### Diff 算法的前置条件：React 如何触发更新？

在深入讲解 Diff 算法的具体策略之前，我们需要理解一个重要问题：

**现在（React 18+）UI 的改变是比较两个虚拟 DOM 从而改变 UI 了，还是新旧 Fiber 树的比较改变 UI？**

#### React 16 及以前（传统虚拟 DOM 更新机制）

```
组件状态改变 → 创建新的虚拟 DOM → 新旧虚拟 DOM 比较（Diff）→ 生成 DOM 更新指令 → 应用到真实 DOM
```

**关键问题**：

- **同步过程**：整个 Diff 过程是同步的，会阻塞主线程，如果组件复杂，会影响用户体验（卡顿）
- **优先级问题**：所有更新都被同等对待，无法区分重要性和紧急程度

#### React 16+（Fiber 架构下的更新机制）
...
【构建阶段 - 可中断】
  • React 遍历新的 Fiber 树
  • 比较新 Fiber 树与旧 Fiber 树（通过 alternate 指针）
  • 计算需要进行的 DOM 操作（插入、删除、更新）
  • React 可以中断这个过程，处理高优先级任务（如用户交互），之后可以恢复
    ↓
【提交阶段 - 不可中断】
  • 一次性将所有 DOM 操作应用到真实 DOM
  • 触发 useEffect 等副作用
  • 旧的 Fiber 树被标记为不可用，新的 Fiber 树成为"当前"的 Fiber 状态
```

**关键变化**：

1. **Fiber 树取代虚拟 DOM 树成为更重要的结构**：React 用 Fiber 节点来表示 UI 的工作单元，新旧 Fiber 树之间的比较驱动了 DOM 的更新
2. **构建阶段可中断**：React 可以在 render 阶段中中断耗时的计算任务，让出主线程，使得用户体验更好
3. **并发渲染**：Fiber 开启了并发渲染的可能性，React 可以在后台准备多个 UI 更新，然后以一种流畅的方式进行切换
4. **优先级和调度**：Fiber 架构允许优先处理某些更新（Transition、Sync 等）

#### 核心结论

**在 React 18+ 中，UI 更新的核心是**：

1. React 基于状态变化构建新的 Fiber 树（工作 Fiber 树）
2. React 通过比较新的 Fiber 树和旧的 Fiber 树（主要是比较 Fiber 节点里的信息）来确定需要进行的 DOM 操作
3. DOM 操作在 commit 阶段一次性应用到 DOM。这个阶段是同步的，但因为计算发生在后台，所以不会阻塞 UI 渲染太多时间

**更精确的解释**：

在 React 18+ 中，比较仍然存在，但它在 Fiber 架构中得到了重新组织和增强：

- **Fiber 节点**：比较的是新旧 Fiber 树中的 Fiber 节点
- **副作用管理**：比较 Fiber 的 `effectTag` 属性，标记了需要进行哪些 DOM 操作（插入、更新、删除）
- **工作单元**：Fiber 将更新过程分解成工作单元，并使用优先级、中断和恢复等技术来优化更新

虽然虚拟 DOM 仍然是 React 中的一个重要概念，它描述了 UI 的状态，但 **Fiber 架构是核心**，它控制了更新的调度、性能和并发。新旧 Fiber 树之间的比较（更确切地说，Fiber 节点之间的比较）是触发 UI 变化的根本原因。
...
### 3. Element Diff（元素 Diff）

**element diff**: **节点比较**，对于同一层级的一组子节点，通过唯一的 **key** 进行比较。

当所有节点处于同一层级时，React 提供了三种节点操作：**插入（INSERT_MARKUP）**、**移动（MOVE_EXISTING）**、**删除（REMOVE_NODE）**

- **INSERT_MARKUP（插入）**：新的组件类型不在旧集合里，即是全新的节点，需要对新节点执行插入操作。
  - 示例：节点 `C` 不在旧集合 `A、B` 中，需要插入

- **MOVE_EXISTING（移动）**：在旧集合里有新组件类型，且 element 是可更新的类型，React 已经调用了 `receiveComponent`，这种情况下 `prevChild=nextChild`，就需要做移动操作，可以复用以前的 DOM 节点。
  - 示例：当组件 `D` 在旧集合 `A、B、C、D` 中，且集合更新时，`D` 没有发生更新，只是位置发生了改变，如：`A、D、B、C`，`D` 的位置从 4 变成了 2
  - **传统 diff 的做法**：会让旧集合的第二个 `B` 和新集合的 `D` 做比较，删除第二个 `B`，再插入 `D`
  - **React diff 的做法**：通过 `key` 来进行直接移动，避免不必要的删除和插入操作

- **REMOVE_NODE（删除）**：旧组件类型在新集合里也有，但对应的 element 不同则不能直接复用和更新，需要执行删除操作；或者旧组件不在新集合里的，也需要执行删除操作。
  - 示例：组件 `D` 在旧集合 `A、B、C、D` 中，如果集合变成了新集合 `A、B、C`，`D` 就需要删除
  - 如果 `D` 的节点发生改变，不能复用和更新，此时会删除旧的 `D`，再创建新的
