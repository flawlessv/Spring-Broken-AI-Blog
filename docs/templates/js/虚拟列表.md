...
虚拟列表的核心思想是：**只渲染可视区域的DOM，对不可视区的DOM不渲染或者部分渲染**的技术。
...
```
滚动容器 (height: 500px, overflow: auto)
│
├─ 占位层 (height: 500,000px)  ← 总高度 = 10000项 × 50px
│  撑开滚动条，让用户可以滚动
│
└─ 渲染层 (position: absolute, transform: translateY(25000px))
   │
   ├─ 第500项 (实际渲染)  ← 偏移后对齐到可视区域
   ├─ 第501项 (实际渲染)
   ├─ 第502项 (实际渲染)
   └─ ... (共20项)

计算过程：
1. scrollTop = 25000px (用户滚动到的位置)
2. startIndex = 25000 / 50 = 500
3. startOffset = 500 × 50 = 25000px
4. 只渲染第500-519项，共20项
```
...
### startIndex 的计算为什么用 Math.floor？

例如当 `scrollTop = 120px`，`itemSize = 50px` 时：
...
在 React 中，我们通过 **ref 回调**在元素挂载时进行测量：
...
当第 N 项的高度变化时，第 N+1 项及之后所有项的 `start` 和 `end` 都需要重新计算。TanStack Virtual 采用 **增量更新策略**：
...
const DynamicVirtualList: React.FC<Props> = ({
  ...
  const resizeItem = useCallback((index: number, newSize: number) => {
    ...
    if (measurements[index].start < scrollTopRef.current && containerRef.current) {
      containerRef.current.scrollTop += delta;
    }
  ...
  const binarySearch = (scrollTop: number): number => {
    const measurements = measurementsRef.current;
    let start = 0, end = measurements.length - 1, result = 0;
    while (start <= end) {
      const mid = Math.floor((start + end) / 2);
      if (measurements[mid].end > scrollTop) {
        result = mid;
  ...
  const actualStartIndex = Math.max(0, startIndex - overscan);
  const endIndex = Math.min(startIndex + visibleCount + overscan, listData.length);
  ...
  return (
    ...
    >
      ...
      <div style={{ height: listHeight }} />
      {/* 渲染层 */}
      <div style={{ position: "absolute", top: 0, transform: `translateY(${startOffset}px)` }}>
...
**这些计算逻辑是纯 JavaScript，与你用 React 还是 Vue 毫无关系！**
...
- [LogRocket - Virtual scrolling: Core principles](https://blog.logrocket.com/virtual-scrolling-core-principles-and-basic-implementation-in-react/)
- [150 lines or less - implementing virtual scroll](https://dev.to/anishkumar/150-lines-or-less-implementing-virtual-scroll-for-web-from-scratch-4363)
